...
export class XMoney {
    ...
    constructor(config: XMoneyConfig | string) {
        ...
        if (!new RegExp(`^sk_(${TEST_ENV}|${LIVE_ENV})_`).test(this.secretKey)) {
            throw new Error(`Invalid secret key. It must start with 'sk_${TEST_ENV}_' or 'sk_${LIVE_ENV}_'.`);
        }
    ...
    public async requestPaginated<T>(method: string, url: string, data?: any, params?: any): Promise<{ data: T[], pagination: Pagination }> {
        const response = await this.client.request<APIResponse<T[]>>({
            method,
            url,
            data,
    ...
    public requestAutoPaginated<T>(method: string, url: string, params?: any): AsyncIterableIterator<T> {
