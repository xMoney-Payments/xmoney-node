...
program
    ...
    .requiredOption('--secret-key <key>', 'xMoney Secret Key', (value) => {
        if (!new RegExp(`^sk_(${TEST_ENV}|${LIVE_ENV})_`).test(value)) {
            console.error(`error: option '--secret-key <key>' argument '${value}' is invalid. It must start with 'sk_${TEST_ENV}_' or 'sk_${LIVE_ENV}_'.`);
            process.exit(1);
        }
        return value;
    });
...
cards
    ...
    .action(createAction(async (client, options) => {
